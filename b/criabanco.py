...
# Cria o banco de dados bdviagens
def criaDatabase():
    conn=psycopg2.connect(database='postgres', host = 'localhost', user = b.inicial.usuario, password = b.inicial.senha)
    conn.autocommit = True
    cursor = conn.cursor()
    sql = 'CREATE database bdviagens'
    cursor.execute(sql)
    conn.close()

# Cria e povoa a tabela CLIENTE
def criaCliente():
    sql = 'DROP TABLE IF EXISTS CLIENTE CASCADE'
    criar_db(sql)
...
# Cria e povoa a tabela VIAGEM
def criaViagem():
    sql = 'DROP TABLE IF EXISTS VIAGEM CASCADE'
    criar_db(sql)
...
# Cria e povoa a tabela ATIVIDADES
def criaAtividades():
    sql = 'DROP TABLE IF EXISTS ATIVIDADES CASCADE'
    criar_db(sql)
...
# Cria e povoa a tabela ATIVIDADE_VIAGEM
def criaAtividadeViagem():
    sql = 'DROP TABLE IF EXISTS ATIVIDADE_VIAGEM CASCADE'
    criar_db(sql)

    sql = '''CREATE TABLE ATIVIDADE_VIAGEM
    (
    ...
    sql = '''INSERT INTO ATIVIDADE_VIAGEM VALUES
    ('2', '1'),
    ('5', '2'),
    ('6', '3'),
    ('8', '4'),
...
# Cria e povoa a tabela RESERVA
def criaReserva():
    sql = 'DROP TABLE IF EXISTS RESERVA CASCADE'
    criar_db(sql)
...
# Cria e povoa a tabela RESERVA_CLIENTE
def criaReservaCliente():
    sql = 'DROP TABLE IF EXISTS RESERVA_CLIENTE CASCADE'
    criar_db(sql)

    sql = '''CREATE TABLE RESERVA_CLIENTE
    (
    ...
    sql = '''INSERT INTO RESERVA_CLIENTE VALUES
    ('22427362808', '1'),
    ('02543856722', '2'),
    ('61663018200', '3'),
    ('80664056776', '4'),
...
# Cria e povoa a tabela ACOMPANHANTE
def criaAcompanhante():
    sql = 'DROP TABLE IF EXISTS ACOMPANHANTE CASCADE'
    criar_db(sql)

    sql = '''CREATE TABLE ACOMPANHANTE
    (
        reservaid		INTEGER			NOT NULL,
        pnome			VARCHAR(15)		NOT NULL,
        unome			VARCHAR(15)		NOT NULL,
...
# Cria e povoa a tabela PASSAGEM
def criaPassagem():
    sql = 'DROP TABLE IF EXISTS PASSAGEM CASCADE'
    criar_db(sql)
    ...
    ('4', 'Ida', 'LATAM', 'BSB', '2022-06-05 14:30:01', '449', '34', '3'),
    ('5', 'Ida-Volta', 'GOL', 'SSA', '2022-06-06 06:01:01', '1044', '21', '4'),
